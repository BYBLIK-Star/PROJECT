...
class PrisonersApp(ctk.CTk):
    ...
    def _empty_stats_payload(self) -> Dict[str, object]:
        return {
            "total_games": 0,
            "wins": 0,
            "losses": 0,
    ...
    def _load_stats_data(self) -> Dict[str, object]:
        if not STATS_FILE.exists():
            return self._empty_stats_payload()
        try:
            loaded = json.loads(STATS_FILE.read_text(encoding="utf-8"))
            if isinstance(loaded, dict):
                base = self._empty_stats_payload()
                base.update(loaded)
                by_n = base.get("by_n")
    ...
    def _save_stats_data(self) -> None:
        try:
            STATS_FILE.write_text(
                json.dumps(self.stats_data, ensure_ascii=False, indent=2),
                encoding="utf-8",
            )
        except Exception:
    ...
    def _record_completed_game(self, won: bool, total_prisoners: int, saved_prisoners: int) -> None:
        ...
        self.stats_data["total_games"] = int(self.stats_data.get("total_games", 0)) + 1
        if won:
            self.stats_data["wins"] = int(self.stats_data.get("wins", 0)) + 1
        else:
            self.stats_data["losses"] = int(self.stats_data.get("losses", 0)) + 1
        self.stats_data["saved_prisoners"] = int(self.stats_data.get("saved_prisoners", 0)) + saved
        self.stats_data["lost_prisoners"] = int(self.stats_data.get("lost_prisoners", 0)) + lost

        by_n = self.stats_data.setdefault("by_n", {})
        if not isinstance(by_n, dict):
            by_n = {}
            self.stats_data["by_n"] = by_n
        ...
        if won:
            n_data["wins"] = int(n_data.get("wins", 0)) + 1
        else:
            n_data["losses"] = int(n_data.get("losses", 0)) + 1
    ...
    def run_stats(self) -> None:
        ...
        total_games = int(self.stats_data.get("total_games", 0))
        wins = int(self.stats_data.get("wins", 0))
        losses = int(self.stats_data.get("losses", 0))
        saved_total = int(self.stats_data.get("saved_prisoners", 0))
        lost_total = int(self.stats_data.get("lost_prisoners", 0))
        win_rate = (wins / total_games * 100) if total_games else 0.0

        if self.stats_kpi_labels:
            self.stats_kpi_labels["games"].configure(text=str(total_games))
            self.stats_kpi_labels["win_rate"].configure(text=f"{win_rate:.1f}%")
            self.stats_kpi_labels["wins"].configure(text=str(wins))
            self.stats_kpi_labels["losses"].configure(text=str(losses))

        by_n = self.stats_data.get("by_n", {})
        if not isinstance(by_n, dict):
            by_n = {}
        ...
        for idx, card_n in enumerate(card_keys):
            ...
            n_stats = by_n.get(str(n_value), {})
            if not isinstance(n_stats, dict):
                n_stats = {}

            n_games = int(n_stats.get("games", 0))
            n_wins = int(n_stats.get("wins", 0))
            n_losses = int(n_stats.get("losses", 0))
            n_saved = int(n_stats.get("saved_prisoners", 0))
            n_lost = int(n_stats.get("lost_prisoners", 0))
            n_win_rate = (n_wins / n_games * 100) if n_games else 0.0

            labels["title"].configure(text=f"{n_value} заключенных")
            labels["attempts"].configure(text=f"Попыток: {n_value // 2}")
            labels["games"].configure(text=f"Игр: {n_games} | Побед: {n_wins} | Поражений: {n_losses}")
            labels["prisoners"].configure(text=f"Заключенные: спасено {n_saved} | потеряно {n_lost}")
            labels["win_rate"].configure(text=f"Процент побед: {n_win_rate:.1f}%")
        ...
        if self.stats_fact_label:
            ...
            self.stats_fact_label.configure(
                text=(
                    ...
                    f"В ваших играх суммарно спасено {saved_total}, потеряно {lost_total}."
                )
            )
